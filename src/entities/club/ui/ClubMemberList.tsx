...
function groupByGeneration(members: ClubMember[]): Map<number, ClubMember[]> {
...
export default function ClubMemberList({ members }: ClubMemberListProps) {
  ...
  return (
    <div className="flex flex-col gap-2">
      <span className="text-title-4 text-main-text">팀원</span>
      <div className="flex flex-col gap-1">
        {generations.map((gen) => {
          const genMembers = generationMap.get(gen)!;
          return (
            <div key={gen} className="flex gap-1 text-text-1">
              <span className="text-sub-1 shrink-0">{gen}기 -</span>
              <span>
                {genMembers.map((m, i) => (
                  <span key={m.id}>
        ...
        <p className="text-text-4 text-sub-2 mt-1">
          ※ 보라색 이름은 부장, 차장이에요
        </p>
      </div>
    </div>
